#include "store_replies_CSRC_link.h"
...
int main(int argc, char** argv) {
	...
	mysql_init(&conn);
	if (!mysql_real_connect(&conn, "localhost", "root", "wuhanA2214", "StockInsider_replies_CSRC", 0, NULL, 0)) 
		return -4;
	...
	while (up_to_date == 0 && more == 1) {
		...
		if (more == 1) {
			...
			while (up_to_date == 0 && regexec(&reg_id, raw_html + offset_id, 1, match_id, 0) != REG_NOMATCH && regexec(&reg_date, raw_html + offset_date, 1, match_date, 0) != REG_NOMATCH) {
				memset(reply_id, '\0', 32);
				memset(reply_date, '\0', 16);
				strncpy(reply_id, raw_html + offset_id + match_id[0].rm_so, match_id[0].rm_eo - match_id[0].rm_so);
				strncpy(reply_date, raw_html + offset_date + match_date[0].rm_so + 6, match_date[0].rm_eo - match_date[0].rm_so - 14);

				if (id_exists(reply_id, conn) == 1) up_to_date = 1;
				else {
					store_reply(reply_id, reply_date, conn);
					updates_count++;
				}
